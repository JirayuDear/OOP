...
def get_available_selection_tennis(request, court: str = None, date: str = None, time: str = None):
    ...
    if not court:
        ...
        content.append(Container(Div(f"ท่านกำลังจะจอง: {court} | {date} | {time}"),
                                 Form(
                                    Button(f"ดำเนินการต่อไป{account_id}", type="submit"),
...
async def post(request):
    ...
    if not member:
        ...
        for booking in history:
            date_time_options.append(Option(f"{booking.get_date} {booking.time}", value=f"{booking.get_date} {booking.time}"))
...
async def post(request):
    if request.method == 'POST':
        ...
        form_data = {
            ...
            'gender': form_data.get('gender', 'ไม่ระบุ'),  # ✅ เพิ่ม gender
            'birth_date': form_data.get('birth_date', '')  # ✅ เพิ่ม birth_date
        }
