...
class Bank:
    ...
    def search_account(self, account_no):
        for i in range(len(self.__user_list)):
            for j in range(len(self.__user_list[i].account_list)):
                if self.user_list[i].account_list[j].account_no == account_no:
                    return self.user_list[i].account_list[j]
        return 'Error'
    def search_edc_machine(self, edc_no):
        for i in range(len(self.__edc_list)):
            if self.edc_list[i].edc_no == edc_no:
                return self.edc_list[i]
        return 'Error'
...
class Account:
    def __init__(self, account_no, user, balance: float):
    ...
    def deposit(self, channel_id, amount):
        if amount > 0:
            self.balance += amount
            temp = Transaction('D', channel_id, amount, self.balance)
            self.add_transaction(temp)
            return "Success"
    ...
    def withdraw(self, channel_id, amount):
        ...
        if isinstance(self, CurrentAccount):
            if amount > 0:
                self.balance -= amount
                temp = Transaction('W', channel_id, amount, self.balance)
                self.add_transaction(temp)
                return "Success"
            return "Error : amount must be greater than 0"
        if isinstance(self, FixedAccount):
            from datetime import datetime, timedelta
            if amount > 0:
                if (datetime.now() - self.deposit_date) >= timedelta(days=365):
                    interest = self.balance*(0.025)
                    self.balance += interest
                    temp1 = Transaction('I-', interest, interest, self.balance)
                    self.add_transaction(temp1)
                elif (datetime.now() - self.deposit_date) >= timedelta(days=180):
                    interest = self.balance*(0.0125)
                    self.balance += interest
                    temp2 = Transaction('I-', interest, interest, self.balance)
                    self.add_transaction(temp2)
...
class SavingAccount(Account):
    ...
    def calculate_interest(self, year):
        ...
        self.balance += interest
        temp = Transaction('I-', 'interest', interest, self.balance)
        self.add_transaction(temp)
        return interest
...
class FixedAccount(Account):
    ...
    @deposit_date.setter
    def deposit_date(self, new_date):
        self.__deposit_date = new_date
    ...
    #         self.balance += amount
    #         temp = Transaction('D', channel_id, amount, self.balance)
    #         if (datetime.now() - self.deposit_date) >= 180:
    #             interest = self.balance*((0.0125))
    #             self.balance += interest
    #         if (datetime.now() - self.deposit_date) >= 365:
    #             interest = self.balance*((0.025))
    #             self.balance += interest
    #         temp1 = Transaction('I-', self, amount, self.balance)
    #         self.add_transaction(temp)
    #         self.add_transaction(temp1)
    #         return "Success"
    #     return "Error : amount must be greater than 0"
...
class DebitCard(Card):
    def __init__(self, card_no, account_no, pin, annual_fee = 300):
        super().__init__(card_no, account_no, pin)
        self.__annual_fee = annual_fee
