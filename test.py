...
class System:
    ...
    def add_admin_list(self, admin): 
        return self.__admin_list.append(admin)
    ...
    def validate_data_signup(self, form_data):
        if len(form_data.get('username')) < 4:
    ...
    def find_courtbooking_to_accept(self, name, court_name, date, time):
        ...
        for member_instance in self.__member_list:
            if member_instance.get_name == name:
                result2 = member_instance.add_point(booking.court.point)
                if result2 == "Success":
                    return "Accept reserve complete!!"
    ...
    def find_courtbooking_to_cancel(self, name, court_name, date, time):
        for courtbooking_instance in self.__courtbooking_list:
            if(courtbooking_instance.booker.get_user.name == name and courtbooking_instance.court.court_name == court_name and 
               courtbooking_instance.date_of_booking == date and courtbooking_instance.time == time):
                self.remove_choosing_list(courtbooking_instance)
                courtbooking_instance.change_status_cancel("ยกเลิกสำเร็จ")
    ...
    def create_booking(self, court, date, time, member, total_price, payment_receipt):
        ...
        history = History(court_booking, court_booking.receipt)
        self.add_history(history)
        self.add_choosing_list(history)
        result1 = member.add_booking_history(history)
        print(result1)
        return result1
    ...
    def search_equipment_by_id(self, item_id):
        for equipment in self.get_equipment_list:
            if equipment.get_item_id == item_id:
                return equipment
    def search_admin_by_username(self, username):
        for admin in self.get_admin_list:
            if admin.get_username == username:
                return admin
    ...
    def time_expire_five_minute(self):
        ...
        return expire_str
    def generate_js_countdown(self, expire_minutes=5, account_id=""):
        js = f"""
        var expireTime = new Date(new Date().getTime() + {expire_minutes} * 60000);
        ...
        function updateCountdown() {{
            ...
            if (timeLeft <= 0) {{
                window.location.href = '/home?account_id={account_id}';
            }} else {{
    ...
    def remove_choosing_list(self, history):
        if isinstance(history, CourtBooking):
            for cl in self.__choosing_list:
                if cl.get_court_booking == history:
                    self.__choosing_list.remove(cl)
                    return "Success"
        for cl in self.__choosing_list:
            if cl == history:
                self.__choosing_list.remove(cl)
...
user_admin = User('Admin', 'admin', '00000000', '0863521179', 'ไม่ระบุ', '0000-00-00')
system.add_user_list(user_admin)
member_admin = Admin('Adminaccount', 'adminloginpassword', 'Admin@gmail.com', user_admin)
system.add_admin_list(member_admin)
...
system.add_gift_list(Redeem('ผ้าเย็น', 200, 150,'https://www.kurinothailand.com/wp-content/uploads/2021/12/SoftFresh_Roll.png'))
system.add_gift_list(Redeem('แก้วเก็บความเย็น', 500, 150,'https://yeti-webmedia.imgix.net/m/7be8b14270aa097c/W-site_studio_drinkware_Rambler_35oz_Straw_Mug_Big_Sky_Blue_Front_0165_Primary_B_2400x2400.png?bg=0fff&auto=format&w=311&q=68&h=311&fit=fill&hattr=500px&wattr=500px'))
system.add_gift_list(Redeem('เวย์โปรตีน', 1000, 150,'https://bucket.fitwhey.com/ProductType/f63ebcace4515f94976ae2cc324da1eb.webp'))


# jirayu = system.search_member_by_account_id('00002')
    ...
    hdrs=(
        Style(""" 
                ...
                .button:hover {
                    ...
                    font-weight: bold;
                    transform: scale(110%); /* ขยายขนาดขึ้น 5% */
                    transition: 0.3s ease-in-out;
                }
                ...
                .button_cp:hover {
                    background-color: #7a1212; /* เปลี่ยนสีเมื่อ hover */
                ...
                .homeafterlog {
                    ...
                    padding: 0;
                    text-align: center;
                    background: url('https://images.unsplash.com/photo-1534158914592-062992fbe900?q=80&w=2099&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D') no-repeat center center fixed;
                    background-size: cover;
                    position: relative;
                    overflow: hidden;
                }
                ...
                .homeafteracc {
                    ...
                    padding: 0;
                    text-align: center;
                    background: url('https://scontent.fbkk29-7.fna.fbcdn.net/v/t39.30808-6/482055567_1289265455502123_6898756407003858293_n.jpg?_nc_cat=108&ccb=1-7&_nc_sid=127cfc&_nc_eui2=AeEHn4gY2JQo1j282XfRR_0sGZ2o5egjaWIZnajl6CNpYvtM9CTMkX4jNOf4lPfgm_zG5nkSXnt6jBEcKT8DMe_H&_nc_ohc=E6Jdvu5ipPAQ7kNvgGZFUc2&_nc_oc=AdiMrAnW09qFG0By27GF1_SZGCj97_IFhW8KEp2C092td3AQrGbSLjgUf31oB17j38M&_nc_zt=23&_nc_ht=scontent.fbkk29-7.fna&_nc_gid=AMNN_TnNrZGVXQQYixFdeLg&oh=00_AYFcg1QPoAVswkuAngMUZR_wwo5mdffA13W6_jKYeYjYTg&oe=67D65D1E') no-repeat center center fixed;
                    background-size: cover;
                    position: relative;
                    overflow: hidden;
                }
...
def get():
    return Container(
        ...
        Div(
            Div(Img(src="https://media.discordapp.net/attachments/1202220660613726248/1348657664065933384/IMG_6527.png?ex=67d042d3&is=67cef153&hm=53c2883354e1cbc5948074d8cfa58bc8d2c74e27639185a540336593f2664f7c&=&format=webp&quality=lossless&width=1050&height=1050", 
            style="height: 420px; width: 400px; padding-top: 100px")),
            Div(
            Form(
                 Button("Accept Reserve", type="submit", cls = "button"),
                method="get",
                action="/accept_reserve"
            ),
            Form(
                Button("Rent Equipment", type="submit", cls = "button"),
                method="get",
        ...
        Div(
            Form(
                 Button("Accept Cancel", type="submit", cls = "button"),
                method="get",
                action="/accept_cancel"
            ),
            Form(
                Button("Court Checking", type="submit", cls = "button"),
                method="get",
                action="/court_checking_admin"
            ),
            Form(
                Button("Booking", type="submit", cls = "button"),
                method="get",
                action="/booking_admin"
            ),
            style="display: flex; gap: 10px; justify-content: center;",
            cls="hero"
        ),
...
def get_available_selection_tennis(request, court: str = None, date: str = None, time: str = None):
    ...
    if not court:
        ...
        content.append(Container(H1("ประเภทกีฬา: เทนนิส"),Form(
        Button("Home", type="submit", cls="side-button"),
...
def get_available_selection_football(request, court: str = None, date: str = None, time: str = None):
    ...
    if not court:
        court_options = [Option(c, value=c) for c in bigdata.keys()]
        content.append(Container(H1("ประเภทกีฬา: ฟุตบอล"),Form(
        Button("Home", type="submit", cls="side-button"),
        Input(type="hidden", name="account_id", value=account_id),
        method = "get",
        action = "/home"
...
def get_available_selection_table_tennis(request, court: str = None, date: str = None, time: str = None):
    ...
    if not court:
        court_options = [Option(c, value=c) for c in bigdata.keys()]
        content.append(Container(H1("ประเภทกีฬา: ปิงปอง"),Form(
        Button("Home", type="submit", cls="side-button"),
        Input(type="hidden", name="account_id", value=account_id),
        method = "get",
        action = "/home"
...
def index(sport_type: str = None, selected_date: str = None):
    ...
    if not sport_type:
        content.append(Div(
            H1("DMIS COURT"),
            A("HOME", href="/", style = "text-decoration: None; color: white;"),
            style="display: flex; justify-content: space-between; width: 100%;"
        ))
        content.append(Form(
...
def get_home(request):
    ...
    return Container(
        Script("""
            document.addEventListener("DOMContentLoaded", function () {
                document.body.classList.add("homeafterlog");
            });
        """),
       Div(
            H1("DMIS COURT"),
        ...
        Div(
            ...
            Div(
            Form(
                Button("COUPON ส่วนลด", style="font-weight: bold", cls="button"),
                Input(type="hidden", name="account_id", value=account_id),
                method="get",
                action="/claim-coupon"
            ),
            Form(
                Button("ใช้ POINT เพื่อแลกสินค้าต่าง ๆ", style="font-weight: bold", cls="button"),
                Input(type="hidden", name="account_id", value=account_id),
                method="get",
                action="/redeem"
            ),
        ...
        Div(
            Form(
                Button("BOOK TENNIS COURT", style="font-weight: bold", cls="button"),
                Input(type="hidden", name="account_id", value=account_id),
                method="get",
                action="/booking_tennis_court"
            ),
            Form(
                Button("BOOK TABLE TENNIS COURT", style="font-weight: bold", cls="button"),
                Input(type="hidden", name="account_id", value=account_id),
                method="get",
                action="/booking_table_tennis_court"
            ),
            Form(
                Button("BOOK FOOTBALL COURT", style="font-weight: bold", cls="button"),
                Input(type="hidden", name="account_id", value=account_id),
                method="get",
...
def get_coupons(request):
    ...
    return Container(
        ...
        """),
        Div(
            H1("Claim Your Coupons!", style = "font-size: 50px; color: #ffffff; font-weight: bold; padding-left: 30px"),
            Div(
                A("BACK", href=f"/home?account_id={account_id}"),
                cls="navbar-links"
            ),
            cls="navbar",
        ...
        Div(
            *[
                Div(
                    H3(f"Coupon: {coupon.get_coupon_code} - Discount {coupon.get_coupon_discount}%", style="color: #ffffff"),
            ...
            cls="coupon-grid", style="padding-top: 150px;"
        )
    )
...
def get(request):
    ...
    return Container(
        Script("""
            document.addEventListener("DOMContentLoaded", function () {
                document.body.classList.add("homeafteracc");
            });
        """),
        # แถบด้านข้าง
        Div(
            A("X", href="/home", style="color: #ffffff; text-decoration: none; position: absolute; top: 10px; right: 10px;"),
            H5(f"Hello, {username}", style="color: #ffffff;"),
            ...
            A("Log out", href="/", style="color: #ffffff; text-decoration: none; display: block; margin-top: 580px; position: related;"),
            style="position: fixed; right: 0; top: 0; background-color: #000000; background-color: rgba(0, 0, 0, 0.7); padding: 20px; width: 220px; height: 100vh; box-shadow: -2px 0 5px rgba(0, 0, 0, 0.1); z-index: 9999;"
        )
    )
...
def get(request):
    ...
    return Container(
        ...
        Div(
            ...
            Div(
                Div(
                    Img(src="https://i.pinimg.com/736x/15/0f/a8/150fa8800b0a0d5633abc1d1c4db3d87.jpg", style="width: 80px; height: 80px; border-radius: 50%;"),
                    H5(username, style="text-align: center; font-weight: bold; padding-top: 10px;"),
                    style="display: flex; flex-direction: column; align-items: center;"
                ),
                
                Div(
                H4("ชื่อ: ", style="color: white; text-align: left; padding-left: 80px;"), 
                H4(name, style="text-align: left; color: white;"),
                H4("นามสกุล: ", style="text-align: left; padding-left: 80px; color: white;"), 
                H4(surname, style="text-align: left; color: white;"),
                H4("วันเกิด: ", style="text-align: left; padding-left: 80px; color: white;"), 
                H4(birthdate, style="text-align: left; color: white;"),
                H4("เพศ: ", style="text-align: left; padding-left: 80px; color: white;"), 
                H4(gender, style="text-align: left; color: white;"),
                H4("E-mail: ", style="text-align: left; padding-left: 80px; color: white;"), 
                H4(gmail, style="text-align: left; color: white;"),
                H4("เบอร์โทรศัพท์: ", style="text-align: left; padding-left: 80px; color: white;"),
                H4(phone, style="text-align: left; color: white;"),
                style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;"
            ),

                
                style=" background-color: #000000; background-color: rgba(0, 0, 0, 0.7); padding: 20px; border-radius: 15px; box-shadow: 0px 4px 6px rgba(0,0,0,0.1); width: 50%;"
            ),

            # Sidebar Menu
            Div(
                Button(f"POINT: {point}", cls="button_p"),
                Form(
                    Button("MY COUPON", type="submit", cls="button_p"),
                    Input(type="hidden", name="account_id", value=account_id),
                    method = "get",
                    action = "/my-coupon"
                    ),
                Button(f"DMIS COIN: {dmis_coin}", cls="button_p"),
                Form(
                    Button("แก้ไขข้อมูลส่วนตัว", type="submit", cls="button_p"),
                    Input(type="hidden", name="account_id", value=account_id),
                    method = "get",
                    action = "/edit-profile"
                    ),
            ...
            style="display: flex; justify-content: space-evenly; padding: 20px; padding-top: 165px;"
        ),
...
def get(request):
    ...
    return Container(
        Script("""
            document.addEventListener("DOMContentLoaded", function () {
                document.body.classList.add("homeafterlog-coupon");
            });
        """),
        # Navbar
        Div(
        ...
        # Main content
        Div(
            Div(H2("Quickly! Use me before I expire!", style="font-size: 35px; color: #ffffff; text-align: center; padding-bottom: 25px;"),
            Div(*coupon_cards, cls="coupon-grid"),
            style="padding: 20px; padding-top: 120px;"),
        ),
        # Footer
        Div(
            A("contact us", href="/contact"),
    ...
    return Container(
        ...
        Div(
            ...
            Div(
                ...
                Form(
                    Label("ชื่อ: ", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="name",placeholder=name, type="text", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    Label("นามสกุล", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="surname", placeholder=surname, type="text", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    Label("วันเกิด", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="birth_date",value=birthdate, type="date", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    Label("เพศ", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="gender", placeholder=gender,type="text", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    Label("E-mail", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="gmail", placeholder=gmail, type="email", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    Label("เบอร์โทรศัพท์", style="color: #ffffff; text-align: left; padding-left: 30px;"), Input(name="phone", placeholder=phone, type="text", cls="input", style="height: 30px; background-color: #000000; color: #ffffff; background-color: rgba(0, 0, 0, 0.7); border-color: white;"),
                    
                    
                    Button("ยืนยันการแก้ไข", type="submit", cls="button_p"),
                    Input(type="hidden", name="account_id", value=account_id),
                    method="post",
            ...
            Div(
                Form(
                    Button("ยกเลิกการแก้ไข", type="submit", cls="button_p"),
                    Input(type="hidden", name="account_id", value=account_id),
                    method = "get",
                    action = "/my-profile"
                    ),
            ...
            style="display: flex; justify-content: space-evenly; padding: 20px; padding-top: 120px; align-items: right;"
        ),
...
def get(request):
    ...
    return Container(
        ...
        # Navbar
        Div(
            H1("REDEMPTION", style="font-size:45px;"),
            Div(
                A("BACK", href=f"/home?account_id={account_id}"),
                cls="navbar-links"
            ),
        ...
        Grid(
            *[
                Card(
                    Img(src = gift.get_gift_url, style = "height: 200px;"),  # ใช้ URL ของแต่ละสินค้า
                    ...
                    Form(
                        ...
                        Input(type="hidden", name="account_id", value=account_id),
                        Button("แลกเลย!", type="submit", cls = "button_p"),
...
async def post(request):
    ...
    for equipment in equipment_list:
        equipment_options.append(Div(
            Img(src=equipment.get_image_url, style="height: 150px; width:150px border-radius: 15%;"),
            Label(
                f"อุปกรณ์: {equipment.get_item_name} ราคา: {equipment.get_item_price} บาท/ชั่วโมง", 
                Input(type="checkbox", name="equipment", value=equipment.get_item_id), style="color: #ffffff"), style="display: flex; align-items: center; justify-content: space-evenly color: #ffffff" 
        ))
...
async def post(request):
    ...
    return Container(
        ...
        A("กลับสู่หน้าหลัก", href="/admin", style="color: #3e1a7d"),
    )
...
async def post(request):
    ...
    return Container(
        ...
        Div(H1("Sign Up", style="text-align: center; color: #ffffff; padding-top: 10px; padding-bottom: 10px;"),
        Form(
            Input(type="text", placeholder="Username", name="username", required=True, style="height: 40px;"),
            Input(type="password", placeholder="Password", name="password", required=True, style="height: 40px;"),
            Input(type="password", placeholder="Confirm Password", name="confirm_password", required=True, style="height: 40px;"),
            Select(
                    Option("เพศ", value="", disabled=True, selected=True), 
                    Option("ชาย", value="ชาย"),
                    Option("หญิง", value="หญิง"),
            ...
            Input(type="text", placeholder="ชื่อจริง", name="first_name", required=True, style="height: 40px;"),
            Input(type="text", placeholder="นามสกุล", name="last_name", required=True, style="height: 40px;"),
            Input(type="date", name="birth_date", required=True, id="birth_date", style="height: 40px;"),
            Input(type="text", placeholder="เลขบัตรประชาชน", name="citizen_id", required=True, style="height: 40px;"),
            Input(type="tel", placeholder="เบอร์", name="phone", required=True, style="height: 40px;"),
            Input(type="email", placeholder="Gmail", name="gmail", required=True, style="height: 40px;"),
            Button("Sign Up", type="submit", 
                        style="width:100%; background:#007bff; color:white; padding:12px; border:none; border-radius:5px; font-size:16px; box-shadow: 4px 4px 8px rgba(0,0,0,0.2); cursor:pointer; transition: all 0.3s;",
                        onmouseover="this.style.backgroundColor = '#90EE90';",
                        onmouseout="this.style.backgroundColor = '#007bff';"),
        ...
        style="box-shadow: 0 20px 40px rgba(0, 0, 0, 0.7); width: 450px; margin: auto; padding: 20px; background-color: #000000; background-color: rgba(0, 0, 0, 0.7); border-radius: 15px; box-shadow: 0 20px 40px text-align: center;")
    )
...
async def get(request):
    ...
    return Container(
        ...
        Div(
            H1("Login", style="text-align: center; color: #ffffff;"),
            Form(
                Input(type="text", placeholder="Username", name="username", required=True),
                Input(type="password", placeholder="Password", name="password", required=True),
            ...
            style="width: 350px; margin: auto; padding: 20px; background: #000000; border-radius: 15px; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.7); background-color: rgba(0, 0, 0, 0.7); text-align: center; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);"
        )
    )
...
async def check_login(request):
    ...
    if admin_login_result is True:
        ...
        return Script("window.location.href = '/admin';")  # เปลี่ยนไปหน้า Admin Dashboard
...
async def post(request):
    ...
    return Container(
        ...
        Div(
            Form(
                Button("เลือกคูปอง", type="submit", cls="button_p", style="margin-right: 10px; width: 200px;"),
                Input(type="hidden", name="account_id", value=account_id),
            ...
            Form(
                Button("ยกเลิกการใช้คูปอง", type="submit", id="cancel_coupon", style="margin-right: 10px; width: 200px;", cls="button_cp"),
                Input(type="hidden", name="account_id", value=account_id),
                Input(type="hidden", name="court_id", value=court_id),
                Input(type="hidden", name="date", value=date),
                Input(type="hidden", name="time", value=time),
        ...
        Form(
            ...
            Button("ชําระเงินด้วย QRCODE", type="submit", cls="button_p", style = "width: 400px"),
            action="/QRCODE",
            method="post"
        ),
        ...
        Form(
            ...
            Button("ชำระเงินด้วย DMIS COINS", type="submit", cls="button_p", style = "width: 400px"),
            action="/Dmis_pay",
            method="post"
        ),
...
async def post(request):
    ...
    return Container(
        ...
        Button(
            "Back",
            onClick=f"window.location.href='Submitnocoupon?account_id={account_id}&court_name={court.court_name}&date={date}&time={time}';"
        ),
    )
    ...
    return Container(
        ...
        H4("คูปองที่เลือก: ", Span(f"{coupon.get_coupon_discount}%", id="coupon_info")),  # ใช้ Span เพื่อให้ JavaScript อัปเดตค่า
            ...
            Div(
                ...
                Form(
                    Button("ยกเลิกใช้คูปอง", type="submit", id="cancel_coupon", style="margin-right: 10px; width: 200px;", cls="button_cp"),
                    Input(type="hidden", name="account_id", value=account_id),
                    Input(type="hidden", name="court_id", value=court_id),
                    Input(type="hidden", name="date", value=date),
                    Input(type="hidden", name="time", value=time),
...
async def post(request):
    ...
    merch = system.generate_js_countdown(account_id=account_id) + system.progress_bar()
    ...
    return Container(
        ...
        Div(
            Img(src="https://scontent.fbkk29-7.fna.fbcdn.net/v/t39.30808-6/484038296_1289510332144302_7935540787749091850_n.jpg?stp=dst-jpg_p526x296_tt6&_nc_cat=106&ccb=1-7&_nc_sid=127cfc&_nc_eui2=AeHwU3VEH2WN8t6-0qFLmGLCjI36bzAZGxiMjfpvMBkbGDf4SkEa_XwY2mV_LydSZTW-QdnyYyGTaDm619YDTlZs&_nc_ohc=izoS3ZvfdfcQ7kNvgHiasFy&_nc_oc=Adjxwqrvm4--0iOAlfUJdqrktlPX5e3JLCxaASJ3vX92u0uGvcxnnBtSrzhsuyiVbVI&_nc_zt=23&_nc_ht=scontent.fbkk29-7.fna&_nc_gid=AK7KOqllocL71HwSh9JAqNd&oh=00_AYHUw0ovXhdz7IFxqqtSUZwI46McnqXO2BqGhbvAUUKiBQ&oe=67D6EFD1", style="width: 450px; height: 450px; margin-top: 10px;"),
        ...
        Form(
            ...
            Button("ยืนยันการจอง", type="submit", id="confirm_button", 
                   style="background-color: lightgreen; padding: 20px 20px; border: none; border-radius: 5px; font-size: 18px; cursor: pointer; width: 300px;",
    ...
    return Container(
        ...
        Form(
            ...
            Button(
                ...
                style="background-color: #3dcc5c; padding: 20px 20px; border: none; border-radius: 5px; font-size: 18px; cursor: pointer; width: 300px;",
            ),
...
async def post(request):
    ...
    return Titled(
        Div(
            ...
            Form(Button("ยกเลิกการจอง", type="cancel", cls="button_cp", style="font-size: 18px; width: 180px;"), 
                 Input(type="hidden", name="account_id", value=account_id),
                 method="get", action="/choose_order"),
            style="display: flex; gap: 10px; justify-content: center;"
        ),
        Table(
            Tbody(*[
...
def get(request):
    ...
    return Titled(
        ...
        Form(
            Button("Back", type="submit", cls="side-button"),
            Input(type="hidden", name="account_id", value=account_id),
            method = "post",
            action = "/bookingHis"
                    ),
...
async def post(request):
    ...
    if selected_booking:
        ...
        system.remove_choosing_list(selected_booking)
...
def get():
    return Container(
        ...
        #     H1("DMIS COURT"),
        #     A("HOME", href="/admin"),
        #     style="display: flex; justify-content: space-between; width: 100%; padding-top:"
        # ),
        
        Form(
            Input(placeholder="Search user", name="username"),
            Button("Search", type="submit"),
            method = "post", style="padding-top: 500px;"
        ),Form(
        Button("Home", type="submit", cls="side-button"),
        method = "get",
        action = "/admin"
...
async def post(request):
    ...
    return Container(
        ...
        H3("พบผู้ใช้", style='text-align: center'),
            Div(
                H2(f"ชื่อผู้ใช้: {member.get_username}", style='text-align: center'),
                style="background-color: e6f3ff; padding: 20px;"
            ),
            Form(
                Button("BOOK TENNIS COURT", type="submit"),
                Input(type="hidden", name="user_name", value=username),
                method="get",
                action="/booking_tennis_court_admin"
            ),
            Form(
                Button("BOOK TABLE TENNIS COURT", type="submit"),
                Input(type="hidden", name="user_name", value=username),
                method="get",
                action="/booking_table_tennis_court_admin"
            ),
            Form(
                Button("BOOKING FOOTBALL COURT", type="submit"),
                Input(type="hidden", name="user_name", value=username),
                method="get",
                action="/booking_football_court_admin"
            ),
            ) if member else Container(
                H1("ไม่พบผู้ใช้", style='text-align: center')
            )
            

@rt("/booking_tennis_court_admin")
def get_available_selection_tennis_admin(request, court: str = None, date: str = None, time: str = None, user_name: str = None):
    account_id = request.query_params.get("account_id") 
    ...
    if not court:
        ...
        action = "/admin"
            ...
            Form(
                Label("เลือกสนาม", 
                    Select(id="court", name="court", 
                         hx_get="/update_tennis_admin", hx_target="#preview", 
                         hx_trigger="change, focusout", 
                         hx_vals=f"js:{{court: event.target.value, account_id: '{account_id}', user_name: '{user_name}'}}"
                                     , *court_options))
        )))
    elif not date:
        available_dates = bigdata.get(court, [])
        ...
        content.append(Form(
            Label("เลือกวันที่", 
                  Select(id="date", name="date", 
                         hx_get="/update_tennis_admin", hx_target="#preview", 
                         hx_trigger="change, focusout", 
                         hx_vals=f"js:{{court: '{court}', date: event.target.value, account_id: '{account_id}', user_name: '{user_name}'}}"
                                    , *date_options))
        ))
    elif not time:
        available_dates = bigdata.get(court, [])
        ...
        content.append(Form(
            Label("เลือกเวลา", 
                  Select(id="time", name="time", 
                         hx_get="/update_tennis_admin", hx_target="#preview", 
                         hx_trigger="change, focusout", 
                         hx_vals=f"js:{{court: '{court}', date: '{date}', time: event.target.value, account_id: '{account_id}', user_name: '{user_name}'}}"
                                     , *time_options))
        ))
    else:
        content.append(Container(Div(f"ท่านกำลังจะจอง: {court} | {date} | {time}"),
...
@rt("/booking_football_court_admin")
def get_available_selection_football_admin(request, court: str = None, date: str = None, time: str = None, user_name: str = None):
    account_id = request.query_params.get("account_id") 
    user_name = request.query_params.get("user_name")
    bigdata = system.avaliable_date_time("ฟุตบอล")
    if not bigdata:
    ...
    if not court:
        ...
        action = "/admin"
            ...
            Form(
                Label("เลือกสนาม", 
                    Select(id="court", name="court", 
                         hx_get="/update_football_admin", hx_target="#preview", 
                         hx_trigger="change, focusout", 
                         hx_vals=f"js:{{court: event.target.value, account_id: '{account_id}', user_name: '{user_name}'}}"
                                     , *court_options))
        )))
...
@rt("/booking_table_tennis_court_admin")
def get_available_selection_table_tennis_admin(request, court: str = None, date: str = None, time: str = None, user_name: str = None):
    account_id = request.query_params.get("account_id") 
    user_name = request.query_params.get("user_name")
    bigdata = system.avaliable_date_time("ปิงปอง")
    if not bigdata:
    ...
    if not court:
        ...
        action = "/admin"
            ...
            Form(
                Label("เลือกสนาม", 
                    Select(id="court", name="court", 
                         hx_get="/update_table_tennis_admin", hx_target="#preview", 
                         hx_trigger="change, focusout", 
                         hx_vals=f"js:{{court: event.target.value, account_id: '{account_id}', user_name: '{user_name}'}}"
                                     , *court_options))
        )))
...
async def get(request: Request):
    ...
    return get_available_selection_tennis_admin(request, court, date, time, user_name)


@rt('/update_football_admin')
async def get(request: Request):
    court = request.query_params.get("court")
    date = request.query_params.get("date")
    time = request.query_params.get("time")
    account_id = request.query_params.get("account_id")
    user_name = request.query_params.get("user_name")
    return get_available_selection_football_admin(request, court, date, time, user_name)

@rt('/update_table_tennis_admin')
async def get(request: Request):
    court = request.query_params.get("court")
    date = request.query_params.get("date")
    time = request.query_params.get("time")
    account_id = request.query_params.get("account_id")
    user_name = request.query_params.get("user_name")
    return get_available_selection_table_tennis_admin(request, court, date, time,user_name)
...
async def post(request):
    ...
    username = form_data.get("username")
    member = system.search_member_by_username(username)
    return Container(H3("พบผู้ใช้", style='text-align: center'),
            Div(
                H2(f"ชื่อผู้ใช้: {member.get_username}", style='text-align: center'),
                style="background-color: e6f3ff; padding: 20px;"
            ),
...
def create_instance(request):
    ...
    court_instance = system.search_court_by_court_name(court_name)
    member = system.search_member_by_username(user_name)
    print(user_name)

    print(court_instance)
    print(date)
    print(time)
    print(member)
    result = system.create_booking(court_instance, date, time, member, court_instance.get_court_price, "ADMIN")
    return Container(
        H1("DMIS COURT"),
        H3("จองสำเร็จ", style="color: green; text-align: center;"),
        A("NEXT", href="/admin", style="display: block; text-align: center;")
    ) if result == "Success" else Container(
        H1("DMIS COURT"),
        H3("จองไม่สำเร็จ", style="color: green; text-align: center;"),
        H3(f"{result}", style="color: green; text-align: center;"),
        A("NEXT", href="/admin", style="display: block; text-align: center;")
    )
     

serve(host="127.0.0.1", port=5556)